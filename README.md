...
- 저장: Supabase PostgreSQL 클라우드 동기화 + 브라우저 로컬 캐시
...
- `src/services/cloudSyncService.ts`: Supabase 클라우드 동기화 어댑터
...
4. 다시 배포하면 닉네임, 프로필, 관심종목, 메모, 판단 기록, 매수/매도 기록, 관심 변화 이벤트가 `sensefolio_notes` 테이블에 저장됩니다.

MVP는 이메일/비밀번호 없이 닉네임으로 불러오는 방식입니다. 공개 배포 후 실제 사용자 데이터 보호가 필요하면 Supabase Auth, Row Level Security 사용자별 정책, 데이터 삭제/백업 기능을 추가해야 합니다.
