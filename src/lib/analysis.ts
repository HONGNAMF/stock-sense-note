...
export function classifyProfile(input: Omit<UserProfile, "riskProfile">): RiskProfile {
  ...
  const hasThemeInterests = input.interests.some((item) => ["AI", "의료", "반도체"].includes(item));
...
export function profileGuide(profile?: RiskProfile) {
  if (!profile) return "닉네임으로 센스폴리오를 만들면 관심사와 기록에 맞춘 참고용 해석을 보여드려요.";
  const guides: Record<RiskProfile, string> = {
    안정형: "가격 변동보다 실적 안정성과 긴 흐름을 먼저 보는 편이 좋아요.",
    성장형: "성장률과 장기 시장 크기를 보되, 과열 신호를 함께 체크해요.",
    테마형: "뉴스와 산업 변화에 민감하므로 기대감과 실제 실적을 나눠서 봐요.",
