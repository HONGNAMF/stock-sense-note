...
const watchReasons = ["싸 보이는데 확신은 없음", "오를 것 같긴 한데 이유는 애매함", "이해는 되는데 가격이 고민됨", "요즘 자주 보여서 계속 보게 됨", "직접 더 알아보고 싶음"];
...
export function ReflectionPanel({ assetKey, assetKind }: { assetKey: string; assetKind: AssetKind }) {
  ...
  function save() {
    const ok = journalService.saveReflection({ ...record, createdAt: new Date().toISOString() });
    if (!ok) {
  ...
  return (
    <section className="mt-7 rounded-3xl bg-white p-5 shadow-soft">
      ...
      {guestMessage ? (
        <div className="mt-3 rounded-2xl bg-lemon/80 p-4 text-sm font-bold text-yellow-950">
      ...
      <div className="mt-4 space-y-4">
        ...
        <button onClick={save} className="h-12 w-full rounded-2xl bg-ink text-sm font-black text-white">
          {saved ? "저장됨" : "판단 기록 저장"}
        </button>
      </div>
    </section>
